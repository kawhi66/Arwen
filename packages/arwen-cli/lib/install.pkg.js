...
function install(packages, isDev) {
    return new Promise(async function(resolve, reject) {
        ...
        let child
        if (await isManagerAvailable('cnpm')) {
            child = spawn('cnpm', ['install'].concat(isDev ? ['--save-dev'] : ['--save']).concat(packages), defaultSpawnConfig)
        } else if (await isManagerAvailable('yarn')) {
            child = spawn('yarn', ['add', '--registry', registry].concat(isDev ? ['--dev'] : []).concat(packages), defaultSpawnConfig)
        } else {
            child = spawn('npm', ['install', '--registry', registry].concat(isDev ? ['--save-dev'] : []).concat(packages), defaultSpawnConfig)
        }
